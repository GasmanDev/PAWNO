/*
GYM SYSTEM UPDATE BY NHATNGUYEN
*/
#include <a_samp>
#include <progress>
...
stock static Float:
	bike_pos[ ][ ] =
{
	{772.172,9.41406,1000.0,90.0},
	{769.242,-47.8984,1000.0,90.0}, 
	{774.625,-68.6406,1000.0,90.0}
};

stock static Float:
	bench_pos[ ][ ] =
{
	{ 773.0491,1.4285,1000.7209, 269.2024 },
	{ 766.3170,-47.3574,1000.5859, 179.2983 }, 
	{ 764.9001,-60.5580,1000.6563, 1.9500 }
};
stock static Float:
	barbell_pos[ ][ ] =
{
	{ 774.42907715,1.88309872,1000.48834229,0.00000000,270.00000000,87.99966431 },
	{ 765.85528564,-48.86857224,1000.64093018,0.00000000,89.49993896,0.00000000 },
	{ 765.34039307,-59.18271637,1000.63793945,0.00000000,89.49993896,181.25012207 } 
};

stock static Float:
	dumb_pos[ ][ ] =
{
	{772.992,5.38281,999.727,270.0},
	{756.406,-47.9219,999.727,90.0},
	{759.18,-60.0625,999.727,90.0}
};

stock static Float:
	dumb_bell_right_pos[ ][ ] =
{
	{772.992,5.18281,999.927,0.0,90.0,90.0} 
};

stock static Float:
	dumb_bell_left_pos[ ][ ] =
{
	{772.992,5.62738,999.927,0.0,90.0,90.0} 
};
...
public OnPlayerConnect( playerid )
{
	...
	TextDrawSetShadow(gym_power[ playerid ], 1);
 	...
 	gym_des[ playerid ] = TextDrawCreate(426.000000, 203.000000, "(Metter):");
	...
	gym_repslabel[ playerid ] = TextDrawCreate(426.000000, 203.000000, "REP:");
	...
	if ( ( newkeys & KEY_SECONDARY_ATTACK ) && !( oldkeys & KEY_SECONDARY_ATTACK ))
	{
		for( new o; o != sizeof run_machine_pos; o ++ )
		{
			if( IsPlayerInRangeOfPoint( playerid, 2.0, run_machine_pos[ o ][ 0 ], run_machine_pos[ o ][ 1 ], run_machine_pos[ o ][ 2 ] ) )
			{
				...
				else if(PLAYER_INTREAM[playerid]==true && BAR_CAN_BE_USED[playerid]==true)
				{
				...
				{
					GameTextForPlayer(playerid,"Xin loi chiec may tap nay~n~ da co nguoi su dung", 5000, 4);
				}
			}
		}
		for( new b; b != sizeof bike_pos; b ++ )
		{
			if( IsPlayerInRangeOfPoint( playerid, 2.0, bike_pos[ b ][ 0 ], bike_pos[ b ][ 1 ], bike_pos[ b ][ 2 ] ) )
			{
				...
				else if(PLAYER_INBIKE[playerid]==true && BAR_CAN_BE_USED[playerid]==true)
				{
		...
		for (new g; g != sizeof bench_pos; g ++)
		{
			if( IsPlayerInRangeOfPoint( playerid, 2.0, bench_pos[ g ][ 0 ], bench_pos[ g ][ 1 ], bench_pos[ g ][ 2 ] ) )
			{
				...
				else if(PLAYER_INBENCH[playerid]==true && BAR_CAN_BE_USED[playerid]==true)
				{
		...
		for (new d; d != sizeof dumb_pos; d ++)
		{
			if( IsPlayerInRangeOfPoint( playerid, 2.0, dumb_pos[ d ][ 0 ], dumb_pos[ d ][ 1 ], dumb_pos[ d ][ 2 ] ) )
			{
				...
				else if(PLAYER_INDUMB[playerid]==true && BAR_CAN_BE_USED[playerid]==true)
				{
	...
	if ( ( newkeys & KEY_SPRINT ) && !( oldkeys & KEY_SPRINT ) )
 	{
	 	if(PLAYER_INTREAM[playerid]==true && BAR_CAN_BE_USED[playerid]==true)
	 	{
			...
			SetPVarInt(playerid,"TapGym",GetPVarInt(playerid,"TapGym")+5);
			if(GetPVarInt(playerid,"TapGym") >= 100 && GetProgressBarValue( player_gym_progress[playerid] ) >= 90)
			{
			CheckGym(playerid);
			}
		}
...
stock CheckGym(playerid)
{
	...
	if(hp < 99.0)
	{
		...
		SetPlayerHealth(playerid,hp+MAUCONG);
		GameTextForPlayer(playerid,"Ban duoc cong ~r~hp", 2000, 4);
		GetPlayerHealth(playerid,hp);
		if(hp > 99.0)
		{
		SetPlayerHealth(playerid,99);
		GameTextForPlayer(playerid,"Da day mau", 2000, 4);
		}
	}
	else if(ar < 99.0)
	{
	    
		SetPlayerArmour(playerid,ar+GIAPCONG);
		GameTextForPlayer(playerid,"Ban da duoc cong ~w~giap", 2000, 4);
		GetPlayerArmour(playerid,ar);
		if(ar > 99.0)
		{
		SetPlayerArmour(playerid,99);
		GameTextForPlayer(playerid,"Da day giap",2000, 4);
		}
	}
	else
	{
		GameTextForPlayer(playerid,"Ban da du suc khoe",2000, 4);
	}
	return 1;
}
